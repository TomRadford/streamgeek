  ...
  .get("/progress/:jobId", uploadTokenAuth, async (c) => {
    ...
    return streamSSE(c, async (stream) => {
      ...
      const cleanupProgressListener = () => {
        ...
        c.req.raw.signal.removeEventListener("abort", cleanupProgressListener);
      };
      ...
      c.req.raw.signal.addEventListener("abort", cleanupProgressListener, {
        once: true,
      });

      try {
        // Sleep for 1 second until the job is complete
        while (!shouldCloseStream) {
